...
const checkDateApf = (() => {
    const now = new Date();
    return (now.getMonth() === 3 && now.getDate() === 1); // 4月1日
});

const cfg = {testMode: false}; // 临时配置，稍后会被实际配置覆盖
let isapf = checkDateApf() || cfg.testMode;
...
// 将launcher数据设置到全局作用域
window.HarmonyOSLauncher = launcherData['HarmonyOSLauncher'] || [];
window.AndroidLauncher = launcherData['AndroidLauncher'] || [];
window.iOSLauncher = launcherData['iOSLauncher'] || [];
window.WindowsLauncher = launcherData['WindowsLauncher'] || [];
window.macOSLauncher = launcherData['macOSLauncher'] || [];
window.LinuxLauncher = launcherData['LinuxLauncher'] || [];

// 保持向后兼容
const HarmonyOSLauncher = window.HarmonyOSLauncher;
const AndroidLauncher = window.AndroidLauncher;
const iOSLauncher = window.iOSLauncher;
const WindowsLauncher = window.WindowsLauncher;
const macOSLauncher = window.macOSLauncher;
const LinuxLauncher = window.LinuxLauncher;
...
// 愚人节彩蛋 - 更新配置
try {
    const actualCfg = read("scripts/config/apf.cfg.jsonc", true);
    if (actualCfg && actualCfg.testMode !== undefined) {
        isapf = checkDateApf() || actualCfg.testMode;
    }
} catch (e) {
    console.warn('无法加载apf配置文件:', e);
}
